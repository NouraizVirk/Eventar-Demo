...
```bash
git clone https://github.com/your-org/eventar.git
cd eventar
```
...
Sample `.env.local` configuration template:
...
DATABASE_URL="postgresql://postgres:password@localhost:5432/eventar_db?schema=public"

# Authentication Secrets
JWT_SECRET="your-super-secret-jwt-key-change-in-production"

# Default System SMTP Credentials (Fallback)
SMTP_HOST="smtp.gmail.com"
SMTP_PORT=587
SMTP_USER="your-email@gmail.com"
SMTP_PASS="your-app-password"
SMTP_FROM="Eventar Notifications <your-email@gmail.com>"

# Google Service Account Credentials (For Sheets API Sync)
GOOGLE_CLIENT_EMAIL="eventar-sync@your-project.iam.gserviceaccount.com"
...
| :--- | :--- | :--- |
| `PrismaClientInitializationError: Can't reach database server` | PostgreSQL service is not running or `DATABASE_URL` credentials in `.env.local` are incorrect. | Verify PostgreSQL daemon status and double-check database connection strings and port numbers. |
| `Google Sheets API Error: The caller does not have permission` | The Google Sheet has not been shared with the Service Account email address. | Open the Google Sheet, click Share, and grant viewer access to `GOOGLE_CLIENT_EMAIL`. |
| `Camera permission denied` when launching PWA Scanner | The browser blocked camera access or the application is served over HTTP instead of HTTPS on mobile. | Ensure camera permissions are allowed in browser settings. For remote testing, run `npm run dev:https` or serve via HTTPS. |
| `Invalid Ticket / Already Used` response on valid scan | The QR code was scanned previously or attendee record was updated to `CHECKED_IN` status. | Inspect the scan audit logs in the organizer dashboard to review timestamp and staff scanner ID details. |
